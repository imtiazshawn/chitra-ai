...
def preprocess_clip(clip_path, required_duration, output_path):
...
def assemble_video(video_map, audio_path, output_path, assets_folder, temp_folder):
    ...
    for i, segment in enumerate(video_map):
        ...
        if not os.path.exists(clip_path):
            ...
            preprocess_clip(clip_path, required_duration, processed_path)
    ...
    for i, processed_clip in enumerate(processed_clips):
        ...
        # Simple scale and crop (no trim needed - already exact duration)
        filter_parts.append(
            f"[{i}:v]scale=1080:1920:force_original_aspect_ratio=increase,"
            f"crop=1080:1920,fps=30,format=yuv420p,setsar=1[v{i}]"
        )

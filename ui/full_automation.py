"""Full Automation - 100% End-to-End Pipeline"""
...
def render_full_automation():
    """Render the Full Automation 100% pipeline page"""
    ...
    with col_left:
        with st.container(border=True):
            ...
            <div style="margin-top: 1rem; padding: 0.75rem; background: rgba(0, 255, 136, 0.1); border: 1px solid #00FF88; border-radius: 6px;">
                <div style="color: #00FF88; font-size: 0.85rem; font-weight: bold; margin-bottom: 0.25rem;">🤖 FULL AUTOMATION</div>
            ...
            # Generate Button
            st.markdown('<div style="margin-top: 1.5rem;">', unsafe_allow_html=True)
            generate_btn = st.button("⚡ START FULL AUTOMATION", use_container_width=True, type="primary")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    # Process Full Automation Pipeline
    if generate_btn:
        if not channel_url:
            ...
            try:
                ...
                log_placeholder.markdown(render_log("// ✓ FULL AUTOMATION COMPLETE"), unsafe_allow_html=True)
                ...
                with output_placeholder.container():
                    ...
                    for i, option in enumerate(metadata['options'], 1):
                        with st.expander(f"Option {i}: {option.get('title', 'N/A')[:50]}...", expanded=(i==1)):
                            st.markdown(f"**Title:**")
